...
import { createSecurityRoleApi, getModulesApi } from '@/lib/fetcher';
import { toast } from 'sonner';
import clsx from 'clsx';

import { useRouter } from 'next/router';
const ACTIONS = [
  { key: 'can_read', label: 'Read', icon: Eye, color: 'text-grey-text border-grey-border', activeColor: 'text-primary border-primary bg-primary/5' },
  { key: 'can_create', label: 'Create', icon: Plus, color: 'text-grey-text border-grey-border', activeColor: 'text-blue-600 border-blue-400 bg-blue-50' },
  { key: 'can_update', label: 'Update', icon: Pencil, color: 'text-grey-text border-grey-border', activeColor: 'text-green-600 border-green-400 bg-green-50' },
  { key: 'can_delete', label: 'Delete', icon: Trash2, color: 'text-grey-text border-grey-border', activeColor: 'text-red-600 border-red-400 bg-red-50' },
];
...
export default function AddRole() {
  ...
  // Fetch modules for the permissions matrix
  useEffect(() => {
  ...
  const reset = () => {
    ...
    Object.keys(resetPerms).forEach(k => {
      resetPerms[k] = { can_read: false, can_create: false, can_update: false, can_delete: false };
    });
  ...
  const handlePermissionChange = (moduleKey, action, checked) => {
    setPermissions(prev => {
      ...
      // Auto-check read if create/update/delete is checked
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      const res = await createSecurityRoleApi({
      ...
      if (res.error || (res.data && !res.data.success)) {
        const errorMsg = res.error?.message || res.data?.message || 'Failed to create role';
        setError(errorMsg);
    ...
    <div className="flex flex-col h-full animate-in fade-in" aria-labelledby={titleId}>
      ...
      <div className="flex shrink-0 items-center justify-between bg-white px-6 py-4 border-b border-grey-border z-10">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-grey-muted mt-0.5">Define new custom security role and access rights</p>
          </div>
        </div>
        <div className="flex items-center gap-3">
          <Button variant="secondary" onClick={handleClose} text="Cancel" />
          <Button variant="primary" type="submit" form="roles-add-form" text={isSubmitting ? "Saving..." : "Save Role"} disabled={isSubmitting} />
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto py-4">
        <div className="max-w-7xl mx-auto flex flex-col gap-6">
          <form id="roles-add-form" className="flex flex-col gap-4" onSubmit={submit}>
            {/* Role Settings */}
