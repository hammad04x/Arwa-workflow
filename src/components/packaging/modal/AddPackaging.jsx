  ...
  const submit = async (e) => {
    ...
    try {
      ...
      if (res.data && res.data.success) {
        ...
        const errorMsg = res.error?.message || res.data?.message || 'Failed to create packaging';
        setError(errorMsg);
    ...
    <div className="app-modal-layer" role="presentation">
      ...
      >
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-4 py-4">
          <form id="packaging-add-form" className="flex flex-col gap-4" onSubmit={submit}>
            <Input
              type="text"
              id="packaging-name"
        ...
        <div className="flex shrink-0 flex-col-reverse gap-2 border-t border-grey-border px-4 py-3 sm:flex-row sm:justify-end">
          ...
          <Button variant="primary" type="submit" form="packaging-add-form" className="flex-1" text={isSubmitting ? "Adding..." : "Add packaging"} disabled={isSubmitting} />
        </div>
      </div>
    </div>,
    root
