...
export default function EditCustomisation({ open, model, onClose, onSave }) {
  const titleId = useId();
  ...
  useEffect(() => {
    if (open && model) {
      ...
      for (const key of MODEL_OPTION_KEYS) {
        const mf = model.specs?.find((s) => s.key === key);
        const tf = CUSTOMISATION_SPECS.find((s) => s.key === key);
        o[key] = [...(mf?.options ?? tf?.options ?? [])];
      }
      setOptions(o);
      setError(null);
    }
  ...
  const submit = (e) => {
    ...
    onSave({ ...model, name: n, code: c, category: cat, specs });
  };
    ...
    <div className="app-modal-layer" role="presentation">
      ...
      >
        <div className="flex shrink-0 items-center justify-between border-b border-grey-border px-4 py-3">
          <h2 id={titleId} className="text-base font-bold text-grey-text-strong">Edit model</h2>
          <button
            type="button"
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-4 py-4">
          <form id="edit-model-form" className="space-y-4" onSubmit={submit}>
        ...
        <div className="flex shrink-0 flex-col-reverse gap-2 border-t border-grey-border px-4 py-3 sm:flex-row sm:justify-end">
          ...
          <Button variant="primary" type="submit" form="edit-model-form" className="flex-1" text="Save changes" />
        </div>
      </div>
    </div>,
    root
